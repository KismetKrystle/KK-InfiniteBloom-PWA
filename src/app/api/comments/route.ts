  ...
  const comments = await sql`
    SELECT
      ...
      EXISTS (
        ...
        AND l2.user_id = (SELECT id FROM user_profiles WHERE auth_user_id = ${session.user.id})
      ) AS liked_by_me,
      c.user_id = (SELECT id FROM user_profiles WHERE auth_user_id = ${session.user.id}) AS is_mine
    FROM comments c
